...
- (id)initWithJSON
{
    ...
    if(self) {
        ...
        NSURLRequest *request = [[NSURLRequest alloc] initWithURL:url cachePolicy:0 timeoutInterval:100];
        ...
        AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
            ...
            self.highTemp = [JSON[@"report"][@"max_temp"] stringValue];
            self.lowTemp = [JSON[@"report"][@"min_temp"] stringValue];
